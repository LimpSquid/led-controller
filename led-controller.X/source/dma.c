...
#define DCHCON_CHBUSY_MASK          0x00008000
...
#define DCHINT_CHBCIF_MASK          0x00000008
#define DCHINT_ENABLE_BITS_MASK     0xffff0000
#define DCHINT_FLAG_BITS_MASK       0x0000ffff
#define DCHINT_FLAG_BITS_MASK       0x0000ffff
...
{    
    ...
    atomic_reg_ptr_clr(dma_int->iec, dma_int->enable_mask);
    atomic_reg_ptr_clr(dma_int->ifs, dma_int->flag_mask);
    ...
    if(atomic_reg_value(dma_reg->dchint) & DCHINT_ENABLE_BITS_MASK)
        atomic_reg_ptr_set(dma_int->iec, dma_int->enable_mask);
}
...
void dma_configure_start_event(struct dma_channel* channel, struct dma_irq event)
{
    ...
    atomic_reg_clr(dma_reg->dchecon, DCHECON_CHSIRQ_MASK);
    atomic_reg_clr(dma_reg->dchecon, DCHECON_SIRQEN_MASK);
    if(event.enable) {
        atomic_reg_set(dma_reg->dchecon, ((unsigned int)event.irq_vector << DCHECON_CHSIRQ_SHIFT) & DCHECON_CHSIRQ_MASK);
...
void dma_configure_abort_event(struct dma_channel* channel, struct dma_irq event)
{
    ...
    atomic_reg_clr(dma_reg->dchecon, DCHECON_CHAIRQ_MASK);
    atomic_reg_clr(dma_reg->dchecon, DCHECON_AIRQEN_MASK);
    if(event.enable) {
        atomic_reg_set(dma_reg->dchecon, ((unsigned int)event.irq_vector << DCHECON_CHAIRQ_SHIFT) & DCHECON_CHAIRQ_MASK);
...
bool dma_busy(struct dma_channel* channel)
{
    ...
    return atomic_reg_value(channel->dma_reg->dchcon) & DCHCON_CHBUSY_MASK;
}
...
static void dma_handle_interrupt(struct dma_channel* channel)
{
    ...
    if(int_flags & DCHINT_CHBCIF_MASK) {
        channel->block_transfer_complete(channel);
        atomic_reg_clr(channel->dma_reg->dchint, DCHINT_CHBCIF_MASK);
    }
}
